...
After cloning, run `nix develop` once to install the pre-commit / pre-push git hooks (see [Workflow](#workflow) below).
...
| Module / package / overlay evaluation | `pre-push` → `nix flake check --no-warn-dirty` | `nix flake check --no-warn-dirty` |
| Authoritative gate | hooks + `just check-all` | `.github/workflows/check.yml` (ubuntu + determinate-nix-action v3 + magic-nix-cache v14) |

`nix flake check` cannot use `--no-build` because the catppuccin home-manager bottom module goes through IFD; magic-nix-cache already keeps the build cost negligible.
...
| [cachix/git-hooks.nix](https://github.com/cachix/git-hooks.nix) | Declarative pre-commit / pre-push hooks |
